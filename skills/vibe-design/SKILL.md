---
name: vibe-design
description: This skill should be used when the user asks to "design a feature", "create architecture", "plan a system", "write a design doc", "how much design do I need?", "should I write a spec?", "is this over-engineered?", "brainstorm an idea", "let's explore this idea", "설계해줘", "아키텍처 잡아줘", "설계 범위 정해줘", "기획 문서 써야 할까?", "설계 문서 만들어줘", "아이디어 좀 정리하자", "이거 어떻게 만들면 좋을까". Guides idea exploration and design scope to produce exactly the right amount of specification for vibe coding, preventing both over-specification and under-specification.
---
...
Turn ideas into the right amount of design specification for vibe coding. This skill covers the full spectrum: from rough idea exploration to structured design documents.

Design = Decisions + Constraints + Milestones. Never pseudocode, function signatures, or exhaustive error paths.
...
Before asking anything, understand the current project state first.

- Check existing files, docs, recent commits to understand what already exists
- Identify relevant patterns, conventions, and constraints in the codebase
- This context informs what questions to ask and what approaches to propose

## Step 0.5: Confirm Input Documents
...
## Step 1.5: Idea Exploration & User Dialogue

아이디어를 구체화하고, 설계 문서 작성 전에 핵심 결정을 확정하는 대화 단계.
...
- **접근 방식 제안**: 핵심 구조 결정 시 2-3개 접근 방식을 트레이드오프와 함께 제시하고, 추천안과 이유를 먼저 제시한 뒤 선택받을 것.
- **사용자 응답이 선택지 밖이면 따를 것**: 사용자가 선택지 외 답변을 하면 그것이 결정이다.
- **YAGNI 적용**: 불필요한 기능은 대화 단계에서 적극적으로 걸러낼 것.
...
```
Idea / Requirement received
  → Explore project context (Step 0)
  → 대상 문서 확인 (유저 미지정 시 재질문) (Step 0.5)
  → Scope Check: 설계 불필요 → 스킬 종료, 바로 구현
  → 설계 필요 →
      1. 아이디어 탐색 & 사용자 대화로 핵심 결정 구체화 (Step 1.5)
...
- 프로젝트 컨텍스트(Step 0) 탐색 없이 바로 질문 시작하기
