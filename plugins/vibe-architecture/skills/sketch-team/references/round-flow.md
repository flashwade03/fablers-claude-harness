...
Instructions:
1. Send each Specialist their role label + role description and the decision sheet — ask each for a preliminary domain artifact (with a short 'alternatives considered and rejected' list when relevant)
...
5. Compose the refined artifacts into a unified draft that honours Confirmed Decisions and resolves Open Decisions, with concrete artifacts from each Specialist appearing in their natural section (Data Models, Interfaces, Sequence Diagrams, etc.). Record every rejected-with-reason alternative mentioned by Specialists inline as 'Alternative considered: X — rejected because Y' next to the winning decision. Surface any 'open question for user' concerns raised by Specialists in a dedicated 'Open Questions for User' section.
