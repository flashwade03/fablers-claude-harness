...
| **sketch-team-orchestrator** | Agent Teams ワークフローで**具体的なマルチドメイン設計＋敵対的レビュー**: Specialist Designer 1–3名（data-model / api-surface / protocol など、Lead がタスク別に決定）が具体的なアーティファクトを生成、Challenger が最弱前提 / 代替案 / エッジケース欠漏を突く（`-c` フラグで specialist 毎のペア Critic）、Planner が批判とドメイン横断整合性を統合して合成、Reviewer 2名が具体化フレンドリーな6軸ルーブリックで判定、承認までループ。 |
